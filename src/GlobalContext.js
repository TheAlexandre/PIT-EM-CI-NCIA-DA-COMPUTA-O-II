...
export const GlobalStorage = ({ children }) => {
  ...
  const getProducts = async (category) => {
    const response = await fetch(`https://my-json-server.typicode.com/thealexandre/api/${category}`);
  ...
  React.useEffect(() => {
    async function loadData() {
      const response = await fetch('https://my-json-server.typicode.com/thealexandre/api/Bebidas_Quentes');
      const json = await response.json();
      setData(json);
    }
    loadData();
